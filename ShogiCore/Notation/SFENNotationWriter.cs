...
namespace ShogiCore.Notation {
    ...
    public class SFENNotationWriter : IStringNotationWriter {
        ...
        public string WriteToString(Notation notation) {
            StringBuilder str = new StringBuilder();
            // 局面
            str.Append(ToString(notation.InitialBoard));
            // 指し手
        ...
        /// SFEN棋譜の書き出し(末尾に改行無しの文字列で返す)
        /// </summary>
        public static string ToString(BoardData boardData, bool useStartPos = true) {
            if (boardData == null) {
                // 初期局面
                return useStartPos ? "startpos" : "sfen lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL b - 1";
            } else {
                StringBuilder str = new StringBuilder();
                str.Append("sfen ");
                // 盤面
                for (int rank = 1; rank <= 9; rank++) {
                    for (int file = 9; 1 <= file; file--) { // １段目の左側（９筋側）から駒の種類を書いていきます
                        if (boardData[file, rank] == Piece.EMPTY) {
                            int count = 1;
                            for (file--; 1 <= file; file--) {
                                if (boardData[file, rank] != Piece.EMPTY) break;
                                count++;
                            }
                            file++; // 1個戻す。(´ω`)
                            str.Append(count);
                        } else {
                            str.Append(SFENNotationReader.ToName(boardData[file, rank]));
                        }
                    }
                ...
                str.Append('1');
                // startpos化
                if (useStartPos && string.CompareOrdinal(str.ToString(),
